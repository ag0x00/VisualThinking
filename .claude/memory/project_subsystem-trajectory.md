...
**NEXT (unprioritized — ask the user which):** colorChord/palette tuning (the deferred v1 axis — would let improve repair wrong-chord starts); external-image CV/VLM front-end (the "evaluate an image we didn't make" goal — feeds the SAME operators); the O4 organized-complexity composite (still NOT triggered — weighted sum has never mis-ranked, don't build speculatively); full wiki write-back binding operators to their concept pages + re-scoring those registry rows (see [[Operational Readiness Registry]] §Toolkit operator-implementation layer); injectable operator registry (only when a real second consumer appears). Screensaver (C) remains the eventual test artifact, NOT the goal (see [[test-artifact-vs-workflow]]).
